...
Blockchiffre der L\"ange $n$. Block = Zeilenvektor der L\"ange $n$ \"uber $\Z_k$.
Klartextblock $v$\\
...
\subsection{Mathematische Methoden gebraucht fuer AES} % NEED BETTER TITLE HERE
...
$
\begin{matrix}
	\leftarrow $Erste\ Zeile\ unver\"andert$\\
...
W\"ahle zwei große Primzahlen $p,q (p\neq q)$ (mindestens 500 Bit L\"ange)\\
Bilde $n=p\cdot q$\\
...
$(n,e)=P_A$\\
W\"ahle $d < \varphi(n)$ mit $e\cdot d \equiv 1 (mod\ n)$ (d.h. $\varphi(n) \mid e \cdot d -1, e\cdot d=1+k\cdot \varphi(n)$ f\"ur $k \in \N$)\\
...
\begin{eqnarray*}
	...
	&x^2-1 \in \Z_p [x]&
\end{eqnarray*}	
\fbox{Entweder $a^t\equiv1\ (mod\ p)$ oder $a^{s^i\cdot t}\equiv -1\ (mod\ p)$ f\"ur ein $0 \leq i \leq s$}
...
$w$ $f^n(w)=w_0 \stackrel{sicher}{\rightarrow}$ Id. \"uberpr\"ufer $f$ Einweg.
\begin{enumerate}
...
$A \stackrel{r^d\ mod\ n}{\longrightarrow} B$ \"uberpr\"uft, ob $r^{d^e}\ mod\ n = r\ \leftarrow$ \textbf{Response}\\
