...
\paragraph{Identitätseigenschaft:}
...
Jeder Empfänger muss digitale Signatur verifizieren können.
...
$m > n\ \ m^d\ mod\ n$ $B\ mod\ n$. Ist $m^{'}\ mod\ n = m\ mod\ n$, dann $(m^{'},m^d\ mod\ n)$ gültige Signatur.
...
\begin{description}
	...
	\item[-] Angreife wählt zufällig $y$ und berechnet $y^e\mod\ n = z$\\
...
Eine \textbf{kryptographische Hashfunktion} ist eine Hashfunktion, die folgende Bedinungen erfüllt.
\begin{enumerate}
	\item $H$ ist Einwegfunktion (um Angriffe des zweiten Typs zu vermeiden)
	\item $H$ ist \textbf{schwach kollisionsresistent}, d.h. zu gegebenem $m \in R^{*}$, soll es effizient nicht möglich sein ein $m' \neq m$, mit $H(m)=H(m')$, zu finden. (um Angriffe des ersten Typs zu vermeiden)
\end{enumerate}
Verschärfung von 2.
\begin{enumerate}
	\item[$2'$] $H$ ist \textbf{stark kollisions resistent}, wenn es effizient nicht möglich ist $m \neq m'$ zu finden, mit  $H(m) = H (m')$.
\end{enumerate}
...
$l$ Personen\\
Alle Möglichkeiten $(g_1,g_2,\ldots,g_l), g_i \in \{1,\ldots,m\}$ $m^l$ Möglichkeiten.\\
...
dass zwei gleich sind ungefähr $\frac{1}{2}$.\\ 
...
Nachweise bzw. Überprüfung, dass jemand derjenige ist für den er sich ausgbit.\\
...
$w$ $f^n(w)=w_0 \stackrel{sicher}{\rightarrow}$ Id. überprüfer $f$ Einweg.
\begin{enumerate}
...
$A \stackrel{r^d\ mod\ n}{\longrightarrow} B$ überprüft, ob $r^{d^e}\ mod\ n = r\ \leftarrow$ \textbf{Response}\\
